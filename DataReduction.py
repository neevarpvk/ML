...
# Data Preparation for Machine Learning | Data Reduction
...
df_diabetes = pd.read_csv("/Users/praveenvudumu/desktop/datascience/datasets/diabetes.csv")
df_diabetes_corr = df_diabetes.corr() #correlation matrix and heatmap
df_diabetes_corr
sns.heatmap(df_diabetes_corr, annot = True, cmap = 'YlGnBu')

X = df_diabetes[['Insulin', 'BMI', 'BloodPressure', 'DiabetesPedigreeFunction']]
y = df_diabetes['Age']
features = X.columns
...
df_auto_cleaned #cleaned df from DataCleaning.py
df_auto_cleaned.describe() #all on different scales - should standardize

from sklearn import preprocessing
from sklearn.model_selection import train_test_split
from sklearn.linear_model import LinearRegression

df_auto_cleaned[['Displacement']] = preprocessing.scale(df_auto_cleaned[['Displacement']].astype('float64')) #standardize because all features are on different scales this will help records center around zero
df_auto_cleaned[['Horsepower']] = preprocessing.scale(df_auto_cleaned[['Horsepower']].astype('float64'))
df_auto_cleaned[['Weight']] = preprocessing.scale(df_auto_cleaned[['Weight']].astype('float64'))
df_auto_cleaned[['Acceleration']] = preprocessing.scale(df_auto_cleaned[['Acceleration']].astype('float64'))
df_auto_cleaned[['Age']] = preprocessing.scale(df_auto_cleaned[['Age']].astype('float64'))

X = df_auto_cleaned.drop(['MPG', 'Origin'], axis = 1) #Origin because Categorical, can use one-hot encoding to convert
...
X = df_auto_cleaned.drop(['MPG', 'Displacement','Weight', 'Origin'], axis = 1) #build LR model with features VIF<5
y = df_auto_cleaned['MPG']
...
df_diabetes_cleaned #cleaned df from DataCleaning.py
...
X = df_diabetes_cleaned.drop('Outcome', axis = 1)
...
df_highVar = df_diabetes_cleaned[['Pregnancies', 'Glucose', 'Age', 'Outcome']] #create a new df with high var features to build ML model
...
df_diabetes_cleaned #outcome is categorical
X = df_diabetes_cleaned.drop('Outcome', axis = 1) #Calc for all features
y = df_diabetes_cleaned['Outcome']
