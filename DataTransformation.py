...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul  6 16:19:55 2020
...
X = df.drop('Outcome', axis = 1) #extract features
y = df['Outcome'] #target
...
dataset['breastquad'].unique() #clean df
dataset['breastquad'] = np.where(dataset['breastquad'].str.contains('nan'), 'central', dataset['breastquad'])
dataset.isnull().sum()

encoder = LabelEncoder() #recommended when no.of categories < 2 (0,1); if label encoded 0-9, scikit lib may give more weight to 9
encoder = encoder.fit(dataset['irradiat'])
dataset['irradiat'] = encoder.transform(dataset['irradiat'].astype(str))
...
dataset_encoded = pd.get_dummies(dataset['breastquad'])
